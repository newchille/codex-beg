...
Status: **PUBLIC RELEASE + HOMEBREW INSTALL VERIFIED**
...
- 2026-08-26 tray lifecycle repair: clicking the tray icon opens only the tray menu and does not bring the window forward; `Open Codex BEG` is the explicit bring-to-front action, while window close and macOS `Cmd+Q` hide to tray. Tray Quit and Dock-context Quit remain the real graceful-exit paths.
- 2026-08-26 local `0.1.2` build: the main window is locked to `1280×820` with resize, maximize, and fullscreen disabled; this build is prepared for handoff but is not installed or published.
- 2026-08-26 local `0.1.2` artifacts: `/private/tmp/codex-beg-build-6/mac-arm64/Codex BEG.app` and `/private/tmp/codex-beg-dmg-0.1.2/Codex-BEG-0.1.2-mac-arm64.dmg` were built and verified; the DMG SHA-256 is `a677d02bdb3dc62f45aa0637221abd5e866f823528f0ab5bbee511e2a5102f53`.
